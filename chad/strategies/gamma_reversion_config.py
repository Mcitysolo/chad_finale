#!/usr/bin/env python3
"""
chad/strategies/gamma_reversion_config.py

Production-grade configuration builder for CHAD GAMMA_REVERSION.

Mirrors the structure of alpha_futures_config.py / gamma_futures_config.py
with validation appropriate for ETF mean-reversion trading.

Public API
----------
- build_gamma_reversion_config() -> StrategyConfig
"""
...
DEFAULT_NOTES: str = "ETF mean reversion engine"
...
class GammaReversionConfigSpec:
...
def _validate_symbol(sym: str) -> None:
    """Strict allow-list validation for GAMMA_REVERSION deployment."""
    allowed = set(DEFAULT_GAMMA_REVERSION_UNIVERSE)
    if sym not in allowed:
        raise ValueError(
            f"Unsupported GAMMA_REVERSION symbol: {sym!r}. "
...
def _validate_universe(universe: Sequence[str]) -> Tuple[str, ...]:
    ...
    if not normalized:
        raise ValueError("GAMMA_REVERSION target_universe cannot be empty.")
...
def _build_validated_spec(
    *,
    ...
    max_gross_exposure: float,
    notes: str,
) -> GammaReversionConfigSpec:
    return GammaReversionConfigSpec(
...
def _load_env_overrides() -> GammaReversionConfigSpec:
    """
    Environment contract
    --------------------
    CHAD_GAMMA_REVERSION_ENABLED=true|false
    CHAD_GAMMA_REVERSION_UNIVERSE=SPY,QQQ,IWM,GLD,TLT
    CHAD_GAMMA_REVERSION_MAX_GROSS_EXPOSURE=0.18
    CHAD_GAMMA_REVERSION_NOTES=ETF mean reversion engine
    """
    enabled = _parse_bool(
        os.getenv("CHAD_GAMMA_REVERSION_ENABLED"),
        default=DEFAULT_ENABLED,
    )
    target_universe = _parse_universe(
        os.getenv("CHAD_GAMMA_REVERSION_UNIVERSE"),
        default=DEFAULT_GAMMA_REVERSION_UNIVERSE,
    )
    max_gross_exposure = _parse_float(
        os.getenv("CHAD_GAMMA_REVERSION_MAX_GROSS_EXPOSURE"),
        default=DEFAULT_MAX_GROSS_EXPOSURE,
    )
    notes = os.getenv("CHAD_GAMMA_REVERSION_NOTES", DEFAULT_NOTES)
...
def build_gamma_reversion_config() -> StrategyConfig:
    """
    Build the canonical GAMMA_REVERSION StrategyConfig.
