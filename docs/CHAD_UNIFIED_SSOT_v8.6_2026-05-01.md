# CHAD Unified SSOT v8.6

**Version:** 8.6
**Date:** 2026-05-01
**Status:** Active — Paper Trading
**Supersedes:** `docs/CHAD_UNIFIED_SSOT_v8.5_2026-04-30.md` (commit `9f55688`, 2026-04-30)

This document is the master reference for the CHAD trading system as it
exists at HEAD (`d35884b`, 2026-05-01). It captures every commit since
v8.5, every wired strategy, every runtime invariant, and the live state
of the machine at the moment of writing. v8.6's signature contribution
is the **2026-04-30/05-01 audit session** — ten forensic audits run
across the live loop, SCR, position guard, profit router, Telegram,
dashboard, reconciliation, edge decay, data feeds, and the D-series
post-merge remediation. Seventeen commits closed gaps that had
accumulated under v8.5's calm-water assumption. Every strategy still
produces signals or fails for a documented expected reason, but now
the *paths around* the strategies — alerts, halts, reconciliation,
shutdown, and feed health — match the strategies' rigor.
...
| Predecessor | v8.5 — `docs/CHAD_UNIFIED_SSOT_v8.5_2026-04-30.md` (`9f55688`) |
| Repository HEAD at write time | `d35884b` — *"Fix: disable per-fill Telegram alerts — too noisy; morning brief + EOD recap only"* |
| Branch | `main` |
...
the *paths around* the strategies behave correctly:
...
The system has graduated from "the strategies emit signals" to "the
operator can sleep through the night."
...
**97 `chad-*` units loaded** (53 services + 44 timers) — adds
`chad-feed-watchdog.service` and `chad-feed-watchdog.timer` over v8.5's
95. **14 services running. 0 failed.**
...
In commit order (oldest first since v8.5 was cut). 17 commits since
`9f55688`, all on 2026-04-30 / 2026-05-01.
...
17 commits across the session. Net code change is mostly additive —
guard logic, alert plumbing, watchdog service, hysteresis router. The
phantom-fill normalizer revert in `3be6785` is the only material
...
│  Services: 14 running / 97 loaded / 0 failed                     │
│  Telegram alerts: 7 exception types ON, per-fill OFF             │
└──────────────────────────────────────────────────────────────────┘
```
...
the night" (v8.6). SCR has promoted to CONFIDENT — the long warmup is
over. The regime booster is active for the first time since it was
built. The feed watchdog, SIGTERM handler, atomic position-guard
write, edge-decay halt enforcement, and seven-type Telegram exception
coverage close the last unattended-operation gaps. The profit router
ledger is now drain-aware; beta and amplifier injection are wired but
flagged off pending paper soak. Reconciliation is GREEN with
strategy-tracked opens visible separately. The dashboard reports
single-signal staleness. No silent failures remain in any of the
audited surfaces.
